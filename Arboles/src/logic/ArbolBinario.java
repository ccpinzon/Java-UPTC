...
public class ArbolBinario {
	...
	public void agregarNodo(Producto producto) {
		...
		if (arbolVacio()) {
			...
			while (aux != null) {
				ant = aux;
				if (aux.getProducto().getCodigo()
						.compareTo(producto.getCodigo()) > 0) {
					aux = aux.getLeft();
				} else {
					aux = aux.getRigth();
				}
			}
			if (ant.getProducto().getCodigo().compareTo(producto.getCodigo()) > 0) {
				ant.setLeft(nuevo);
			} else {
				ant.setRigth(nuevo);
			}
	...
	public NodoArbol buscarNodo(String codigo) {
		NodoArbol aux = raiz;
		while (aux != null) {
			if (aux.getProducto().getCodigo().equals(codigo)) {
				return aux;
			}
			if (aux.getProducto().getCodigo().compareTo(codigo) > 0) {
				aux = aux.getLeft();
			} else {
				aux = aux.getRigth();
			}
	...
	public NodoArbol eliminarNodoConHijos(NodoArbol nodo) {
		...
		if (padre != null) {
			padre.setLeft(sustituto.getRigth());
		...
		if (nodo == raiz) {
			...
			NodoArbol padreNodo = buscarPadre(nodo);
			if (padre.getLeft() != null) {
		...
		if(nodo==null){
				...
				while(true){
					if(aux.getLeft()==nodo||aux.getRigth()==nodo){
						...
						if(aux.getProducto().getCodigo().compareTo(nodo.getProducto().getCodigo())>0){
							aux=aux.getLeft();
						}else{
							aux.getRigth();
						}
